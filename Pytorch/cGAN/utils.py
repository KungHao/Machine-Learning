...
def load_checkpoint(G, D, num_epoch):
    G.load_state_dict(torch.load(os.path.join('./checkpoint', 'G_{}.pth'.format(num_epoch))))
    D.load_state_dict(torch.load(os.path.join('./checkpoint', 'D_{}.pth'.format(num_epoch))))
    return G, D